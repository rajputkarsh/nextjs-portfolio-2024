...
export default function AnimatedAvatar() {
  function loadModel() {
    ...
    loader.load(
      ...
      (gltf) => {
        setupScene(gltf);
        (document.getElementById("avatar-loading") as HTMLElement).style.display = "none";
      },
      (xhr) => {
        const percentCompletion = Math.round((xhr.loaded / xhr.total) * 100);
        (document.getElementById(
          "avatar-loading"
        ) as HTMLElement).innerText = `LOADING... ${percentCompletion}%`;
      },
      (error) => {}
    );
  }

  function setupScene(gltf: GLTF) {
